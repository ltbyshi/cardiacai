...
if __name__ == '__main__':
    ...
    parser = subparsers.add_parser('get_image_size',
    ...
    # command: remove_sides
    parser = subparsers.add_parser('remove sides',
                                   help='remove text and black region on the left and right sides of the images')
    parser.add_argument('-i', '--input-file', type=str, required=True,
    ...
    if args.command == 'get_image_size':
        from scipy.misc import imread
        print('filename\theight\twidth')
        for input_file in args.input_files:
            img = imread(input_file)
            print('%s\t%d\t%d'%(input_file, img.shape[0], img.shape[1]))
    elif args.command == 'image_to_hdf5':
        import cv2
        ...
        fout.close()
    elif args.command == 'merge_image_hdf5':
        import numpy as np
        import h5py
        ...
        fout.close()
    elif args.command == 'hdf5_to_image':
        import h5py
        import cv2
        import numpy as np
        ...
        combined = np.zeros((height*nrow, width*ncol, images.shape[3]), dtype=images.dtype)
        for k in range(min(images.shape[0], nrow*ncol)):
            ...
            if args.resize is not None:
                ...
                image = images[k]
            combined[(i*height):((i + 1)*height), (j*width):((j + 1)*width)] = image
    ...
    elif args.command == 'remove_sides':
        import scipy.signal
        import h5py
        import numpy as np
        logger.info('')
    elif args.command == 'download_model':
    ...
    elif args.command == 'augment_images':
        import h5py
        import numpy as np
        from scipy.misc import imsave
    ...
    elif args.command == 'trace_to_mask':
        import h5py
        import numpy as np
        import cv2
