...
def lrp(F, lowest, highest,
        graph=None,
        return_flist=False,
        input_tensor_name='absolute_input',
        output_tensor_name='absolute_output',
        ):
    """
    ...
    for n in traversed:
        ...
        if graph_dict[n].op == "MatMul":
            ...
            if input_tensor_name in graph_dict[n].input:
...
def get_traversed(graph = None, input_tensor_name='absolute_input', output_tensor_name='absolute_output'):
    ...
    return traverse(graph_dict[output_tensor_name], [], graph_dict, input_tensor_name=input_tensor_name), graph, graph_dict, var_dict


def traverse(node, L, graph_dict, input_tensor_name='absolute_input'):
    #Depth First Search the Network Graph
    L.append(node.name)
    if input_tensor_name in node.name:
        return L
    inputs = node.input
    for nodename in inputs:
        if not traverse(graph_dict[nodename], L, graph_dict, input_tensor_name=input_tensor_name) is None:
            return L
    return None
