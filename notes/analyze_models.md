...
bin/analyze.py analyze_relevance --method $method \
    -m output/classify_diseases/vgg16/$classes/cv/0/model \
    -i data/merged/datasets/320x320.h5 --input-dataset X \
    -y output/classify_diseases/vgg16/$classes/targets --target-dataset y \
    --image-id-file output/classify_diseases/vgg16/$classes/cv_split \
    --image-id-dataset /0/test \
    -o output/classify_diseases/vgg16/$classes/cv/0/$method
bin/preprocess.py hdf5_to_image -i output/classify_diseases/vgg16/$classes/cv/0/$method \
    -d X --nrow 20 --ncol 10 --resize 128,128 \
    -o output/classify_diseases/vgg16/$classes/cv/0/preview/${method}.X.png
bin/preprocess.py hdf5_to_image -i output/classify_diseases/vgg16/$classes/cv/0/$method \
    -d relevance_map --nrow 20 --ncol 10 --resize 128,128 \
    -o output/classify_diseases/vgg16/$classes/cv/0/preview/${method}.relevance_map.png
```
